...
node* CreateNode(node *nenode)//创建节点，0表示创建空节点，1表示创建非空节点
 {
   char a;
   int c,i;
    float b;
  ...
  node* CreateList(node *L,int k)//创建一个链表
 {
...
node* AddNode(node*h,node*q,int i)//将一个结点插入到链表的指定位置。
{
...
node* DeleteNode(node *h,int d)//删除指定节点
{
    int j=0;
